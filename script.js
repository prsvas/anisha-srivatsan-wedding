...
function initCounters(){
  const ids = {
    days: ["countDays", "finalCountDays"],
    hours: ["countHours", "finalCountHours"],
    minutes: ["countMinutes", "finalCountMinutes"],
    seconds: ["countSeconds", "finalCountSeconds"]
  };
  ...
  const tick = () => {
    ...
    if (remaining <= 0) {
      setValue(ids.days, "00");
      setValue(ids.hours, "00");
      setValue(ids.minutes, "00");
      setValue(ids.seconds, "00");
      return;
    }

    setValue(ids.days, String(Math.floor(remaining / 86400000)).padStart(2, "0"));
    setValue(ids.hours, String(Math.floor((remaining % 86400000) / 3600000)).padStart(2, "0"));
    setValue(ids.minutes, String(Math.floor((remaining % 3600000) / 60000)).padStart(2, "0"));
    setValue(ids.seconds, String(Math.floor((remaining % 60000) / 1000)).padStart(2, "0"));
  };
