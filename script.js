...
function initCounters(){
  ...
  const format=(d)=>{
    if(d<=0)return "The day has arrived";
  ...
  const tick=()=>{const text=format(target-Date.now()); if(heroEl)heroEl.textContent=text; if(closingEl)closingEl.textContent=text;};
